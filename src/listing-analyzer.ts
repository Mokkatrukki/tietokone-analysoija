...
import { isLaptopListing, isComputerListing } from './utils/categoryUtils';
...
export function analyzeListing(db: HardwareSpecsDB, listing: ToriListing): AnalysisResult | null {
  // Check if this is a computer listing (laptop or desktop)
  if (!isComputerListing(listing)) {
    return null;
  }
  ...
  return {
    listingType: isLaptopListing(listing) ? 'laptop' : 'desktop',
    cpu: cpuSpec ? {
      name: cpuSpec.name,
